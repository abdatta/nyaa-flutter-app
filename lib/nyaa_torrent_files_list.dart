...
class NyaaTorrentFile {
  ...
  NyaaTorrentFile({@required this.name, this.isFolder = false, this.subfiles = const []});
...
class NyaaTorrentFilesList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ExpansionTile(
        title: Text('Files List', style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        return ExpansionTile(
          title: Row(
            children: <Widget>[
              ...
              Flexible(
                child: Text(file.name, style: TextStyle(fontSize: 14))
              )
            ]
          ),
          children: this.buildFilesList(file.subfiles)
            .map((w) => Padding(
              ...
              Flexible(
                child: RichText(
                  text: TextSpan(
                    ...
                    children: <TextSpan>[
                      TextSpan(
                        text: file.name.substring(file.name.lastIndexOf('('), file.name.length),
